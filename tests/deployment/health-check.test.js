...
const { httpHealthCheck } = await import(
  '../../apps/worker/src/deployment/health-check.js'
);
...
describe('httpHealthCheck', () => {
  ...
  it('returns healthy=true on first 200 response', async () => {
    mockFetch([200]);
    const result = await httpHealthCheck({ url: 'http://127.0.0.1:11111/', attempts: 3, intervalMs: 1 });
  ...
  it('returns healthy=true on 302 redirect', async () => {
    mockFetch([302]);
    const result = await httpHealthCheck({ url: 'http://127.0.0.1:11111/', attempts: 1, intervalMs: 1 });
    assert.equal(result.healthy, true);
  });

  it('retries on connection error and eventually succeeds', async () => {
    mockFetch([new Error('ECONNREFUSED'), new Error('ECONNREFUSED'), 200]);
    const result = await httpHealthCheck({ url: 'http://127.0.0.1:11111/', attempts: 5, intervalMs: 1 });
    assert.equal(result.healthy, true);
  });
  ...
  it('returns healthy=false on 500 response throughout', async () => {
    mockFetch([500]);
    const result = await httpHealthCheck({ url: 'http://127.0.0.1:11111/', attempts: 2, intervalMs: 1 });
    assert.equal(result.healthy, false);
