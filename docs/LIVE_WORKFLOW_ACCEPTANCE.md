...
| Web and worker               | Both repository-run processes are active                           | Passed  | `hellodeploy-web` and `hellodeploy-worker` started live (2026-08-06) under their intended identities and passed health/readiness; both are active as transient candidates (not enabled for boot persistence) alongside the still-authoritative PM2 web; the queue remains paused |
...
| Wildcard application ingress | `*.hellodeploy.online` reaches managed project routes              | Not Run | DNS and Cloudflare Tunnel ingress for the wildcard both pass live (2026-08-08); a public probe returns a real TLS-terminated response, confirming the domain and certificate now work end-to-end. Reaching an actual managed project route specifically is unverified — no project has been deployed yet (P3)                                                                        |
