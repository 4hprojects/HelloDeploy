...
| Immutable runtime identity   | Active processes correspond to one reviewed commit or release tag  | Failed  | PM2 started before several later checkout changes; emergency capture and controlled restart on `v0.1.2` are required                                                                                                         |
...
| Backup and restore           | Encrypted backup restores on a second machine                      | Blocked | Hardened capture/verifier and malicious-archive tests pass; exact GPG fingerprint, root-owned off-host destination, verified database snapshot, actual artifact, retrieval check, and second-machine restore remain required |
