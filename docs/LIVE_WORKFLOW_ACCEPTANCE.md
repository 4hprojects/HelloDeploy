...
| Backup and restore           | Encrypted backup restores on a second machine                      | Blocked | Backup tooling is released, but no removable backup or separate recovery-key medium is attached; snapshot, artifact, retrieval, and second-machine restore remain required |
