...
| Wildcard application ingress | `*.hellodeploy.online` reaches managed project routes              | Blocked | Local Cloudflare Tunnel ingress for the wildcard hostname passed live (2026-08-05); the tunnel now has the rule, but the DNS record itself is still absent, so the hostname does not yet resolve publicly                                                                        |
