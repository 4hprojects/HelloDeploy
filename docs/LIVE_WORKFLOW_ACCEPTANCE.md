...
| Session cookie   | `Secure; HttpOnly; SameSite=Strict`                     | Failed | `npm run production:check -- https://hellodeploy.online` still reports `missing secure`; production start now forces production mode locally, but the change must be deployed and rechecked |
