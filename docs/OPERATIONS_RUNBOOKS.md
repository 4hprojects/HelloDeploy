...
The creation command validates the ciphertext with GPG's non-decrypting packet mode. This is intentional: only the public key remains on the pilot, so a normal packet listing attempts decryption and reports a false failure when the secret key is correctly absent. Successful creation is still only the first gate; recovery-key import and the verifier below prove that the artifact can actually be decrypted and checked.

Store the encrypted artifact on the approved off-host medium, unmount and remount or otherwise retrieve it, then run `bash infrastructure/verify-pilot-backup.sh --prompt-passphrase <encrypted-artifact>` from a temporary GPG home containing the passphrase-protected recovered private key. The prompt reads only from the controlling terminal, never places the passphrase in command arguments, unsets it immediately after use, and bounds decryption to two minutes. Omit `--prompt-passphrase` only for a deliberately noninteractive key provider that is already configured to unlock the key. The verifier does not restore files or change services. Retrieval verification on the pilot host proves that the artifact and separately held key are usable; it does not satisfy the cross-host restore gate. Do not proceed based only on successful encryption.
...
Run this after pulling any release that touches the build/deploy pipeline —
step 5 of the Ordered Production Workflow above ("Deploy: ... confirm
routing, non-root identity, loopback binding, ... healthy-release
continuity") in concrete commands. Originally written for the non-root
container change (`docs/phases/phase-2-non-root-containers.md`), which
could not be exercised in the development environment (no Docker socket
access) — keep running it on every pipeline-touching release, not just
that one.
