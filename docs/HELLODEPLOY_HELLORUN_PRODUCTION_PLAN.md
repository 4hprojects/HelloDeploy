...
before now; `infrastructure/activate-dashboard-cutover.sh` and
`revert-dashboard-cutover.sh` were built, fail-closed, mirroring the established
pattern. Cloudflare Tunnel was found routing `hellodeploy.online`/
`www.hellodeploy.online` directly to the PM2 port, bypassing Nginx entirely, and a
stale legacy vhost was already enabled and would have conflicted with the real
platform vhost — both handled as part of the script. The live cutover passed every
stage on the first attempt: `hellodeploy.online`/`www.hellodeploy.online` now serve
via the isolated `hellodeploy-web` through Nginx, confirmed publicly including
proof via the Nginx access log that traffic actually moved, and PM2 was never
stopped. Deliberately exercising the revert script (to satisfy the "restore the
former path" evidence requirement) surfaced a real bug: an unrelated failure —
HelloRun's separate PM2 process crash-looping on a pre-existing port-3000
...
| 2026-08-09 | P2       | `32b5adbc0a434c78cdfc6ed7c2b56492e581d8b6` | Live dashboard traffic cutover              | hellodeploy.online/www now served via isolated hellodeploy-web through Nginx; PM2 never stopped                            | Exercise revert; resume queue                                      | [Worklog](../WORKLOG.md)                         |
| 2026-08-09 | P2       | `961cc8549181f650a7a4d26cac78f8b94b81cc88` | Revert exercise + rollback bug fix          | Unrelated HelloRun failure triggered rollback, which left Nginx/tunnel inconsistent (brief outage); fixed live and in code | Full clean revert blocked on HelloRun's own recovery; resume queue | [Worklog](../WORKLOG.md)                         |
