...
As of 2026-07-31, P1 is complete on the Ubuntu 26.04 pilot. The encrypted P0 capture passed same-host retrieval, and candidate `704cb75a02d76a36a88d155a37052df4464bf1a2` now provides Docker, system Node.js 22, protected configuration, isolated identities, managed-route storage, and disabled systemd units. The inactive verifier and a real helper/web start, readiness, and clean shutdown passed while the worker remained inactive and managed routes remained unchanged. The PM2 dashboard and independent HelloRun fallback still pass publicly. P2 has paused and drained the combined deployment/domain queue; no deployment jobs were present and one valid DNS-verification job remains paused. The next gate reviews and activates the constrained helper while keeping the worker inactive and queue paused, then validates wildcard ingress, isolated services, and rollback before traffic cutover. The release remains **NO-GO for customer application hosting** until routing, real deployments, rollback, authenticated QA, and cross-host restoration pass directly.
