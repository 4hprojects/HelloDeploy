...
As of 2026-08-08, P1 is complete on the Ubuntu 26.04 pilot. The encrypted P0 capture passed same-host retrieval, and the isolated foundation provides Docker, system Node.js 22, protected configuration, isolated identities, and managed-route storage. P2 has paused and drained the combined deployment/domain queue; no deployment jobs were present and one valid DNS-verification job remains paused. The constrained helper is active and enabled, and live route creation, replacement, invalid-candidate restoration, and removal passed without probe residue while the worker remained inactive and the queue paused. The PM2 dashboard and independent HelloRun fallback still pass publicly. Candidate service activation has passed live under intended identities, passing health, readiness, the secure session cookie, worker readiness, and Nginx syntax; both services are active as transient candidates, not yet enabled or receiving dashboard traffic. The wildcard domain was restructured from `*.apps.hellodeploy.online` to `*.hellodeploy.online` after discovering the account's free Cloudflare certificate doesn't cover second-level wildcards; the live migration passed, and a public wildcard HTTPS probe now returns a real TLS-terminated response, confirming that gap is resolved. Two unrelated live issues surfaced and were fixed along the way: a stale systemd `ReadWritePaths` bind-mount on the Nginx helper that broke daily after log rotation, and a `DEPLOYMENT_DOMAIN`/`PLATFORM_SUBDOMAIN_SUFFIX` mismatch that had the repository-run PM2 pilot crash-looping. The immediate next step is dashboard traffic cutover from PM2 to the candidate web service, then gradual queue resume. The release remains **NO-GO for customer application hosting** until routing, real deployments, rollback, authenticated QA, and cross-host restoration pass directly.
