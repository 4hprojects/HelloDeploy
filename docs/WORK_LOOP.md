...
As of 2026-08-06, P1 is complete on the Ubuntu 26.04 pilot. The encrypted P0 capture passed same-host retrieval, and the isolated foundation provides Docker, system Node.js 22, protected configuration, isolated identities, and managed-route storage. P2 has paused and drained the combined deployment/domain queue; no deployment jobs were present and one valid DNS-verification job remains paused. The constrained helper is active and enabled, and live route creation, replacement, invalid-candidate restoration, and removal passed without probe residue while the worker remained inactive and the queue paused. The PM2 dashboard and independent HelloRun fallback still pass publicly. Local wildcard-ingress activation for both connectors on the shared dashboard tunnel has passed live, adding only local Cloudflare Tunnel ingress rules; wildcard DNS remains unchanged and absent. Candidate service activation has also passed live: a first attempt surfaced a real session-write/shutdown-ordering race (graceful shutdown now waits for pending session-store writes before closing the database), and the retry started `hellodeploy-web` and `hellodeploy-worker` under their intended identities, passing health, readiness, the secure session cookie, worker readiness, and Nginx syntax. Both services are active as transient candidates, not yet enabled or receiving dashboard traffic. The immediate next step is adding the authoritative wildcard DNS record, then wildcard HTTPS verification, dashboard traffic cutover, and gradual queue resume follow. The release remains **NO-GO for customer application hosting** until routing, real deployments, rollback, authenticated QA, and cross-host restoration pass directly.
