...
As of 2026-07-14, the current Ubuntu 26.04 laptop remains the live HelloDeploy dashboard pilot and in-place productionization target. Reviewed release `v0.1.5` is published. The encrypted emergency pilot capture passed same-host retrieval and non-restoring verification; cross-host restore remains blocked. A PM2 reload picked up the production-enforcing start scripts and exposed the expected missing-helper blocker, so the combined web/worker entry was stopped after a crash loop. The GitHub App identity and newest key were verified, the protected key was installed, and a separate production web-only PM2 pilot now serves the clean `v0.1.5` checkout. Local/public health, readiness, assets, HSTS, CSP, and `Secure; HttpOnly; SameSite=Strict` pass. This restores the dashboard but does not satisfy Node.js 22 or service-identity normalization: the web still runs under the interactive PM2/Node.js 24 pilot identity and the worker is offline. The next action is prepare-only Docker and isolated service installation, followed by queue control, constrained-helper validation, isolated candidate activation, and the final post-normalization backup. The release remains **NO-GO for customer application hosting** until isolation, routing, real deployments, rollback, authenticated QA, and cross-host restoration pass directly.
