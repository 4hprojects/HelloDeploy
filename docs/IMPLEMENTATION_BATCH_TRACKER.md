...
- [x] Define the exact service, Nginx, tunnel, and repository rollback path while the current dashboard remains available.
...
**2026-07-31 P0 evidence:** PR #11 passed Node.js 22 CI and merged as clean candidate `6d0bf82530d01bb941b6309c83a1a8bde18a4447`. The same commit passed local Node.js 22 clean installation, lint, formatting, configuration validation, 839 tests, production audit, and clean-worktree checks. A fresh bounded host baseline confirmed dashboard health and the expected Docker, identity, helper, route-directory, and wildcard-ingress blockers. Read-only queue inspection found no deployment work and one valid DNS verification waiting for P2. The operator then restored the dashboard frontend release and the independent HelloRun fallback. PR #13 passed Node.js 22 CI and merged the reviewed recovery evidence as candidate `2ed2f4ea390d32267820fee4d854b3aa2f7d11f6`. MongoDB Database Tools `100.17.0` passed the official signing-fingerprint, detached-signature, and archive-path checks before root installation. A compressed database export passed `mongorestore --dryRun` and its protected checksum. The final encrypted artifact captured the database export, environment, dashboard Nginx and tunnel state, supplemental current HelloRun route/token/unit state, release identity, and root-owned rollback instructions. After the backup drive was locked and remounted, its outer checksum passed. The separate recovery media passed its recorded checksum and secret-key import; a temporary root keyring decrypted the artifact, and the bounded verifier passed the member allowlist and every internal checksum. The keyring was removed, both media were safely detached, the repository remained clean, and the complete dashboard and HelloRun public checks still passed. The recovery USB has pre-existing FAT metadata differences but consistent key files; it remains read-only until a second verified key copy permits repair. P0 is Complete. Same-host retrieval does not satisfy the later second-host restore gate.
...
**2026-07-31 routing-foundation evidence:** Reviewed fixes aligned the verifier with
the installed environment, granted the helper only the exact Nginx PID and configured
log files required by this host, and bounded HTTP checks across graceful Nginx reload
convergence. The live worker-identity probe then passed route creation, replacement,
invalid-candidate rejection with prior-route restoration, and removal. The helper
remains active and enabled; the managed include is present; no probe file or listener
remains; the dashboard and HelloRun fallback pass publicly; and the worker and queue
remain inactive/paused. Wildcard DNS is still absent.
...
record added 2026-08-05 found it consistently failed its TLS handshake — the
account's free Cloudflare certificate covers only `hellodeploy.online` and
`*.hellodeploy.online`, not the second-level `*.apps.hellodeploy.online` this
platform used. Restructured to `*.hellodeploy.online` (config defaults, two infra
scripts including a new deactivation script, tests, and docs — Nginx routing is
domain-agnostic, no application logic changed). The live migration passed: the old
wildcard rule was cleanly removed, the new one activated, and its DNS record added.
A first candidate-services retry failed at `worker-ready`, traced to a stale
systemd `ReadWritePaths` bind-mount on the Nginx helper breaking daily after log
rotation (fixed, unrelated to the domain change); the retry after that passed every
stage. A public wildcard HTTPS probe now returns a real TLS-terminated response,
confirming the certificate gap is resolved. Separately, the repository-run PM2
