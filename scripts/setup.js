...
if (!skip('NODE_ENV'))    config.NODE_ENV    = 'production';
if (!skip('PORT'))        config.PORT        = await ask(rl, 'Web port', existing.PORT || '3000');
if (!skip('HOST'))        config.HOST        = await ask(rl, 'Bind host', existing.HOST || '0.0.0.0');

// ── Section 2: platform domain ────────────────────────────────────────────────
console.log(`\n${BOLD}2. Platform domain${RESET}`);
const domain = await ask(rl, 'Platform domain (e.g. hellodeploy.example.com)', existing.PLATFORM_DOMAIN || '');
config.PLATFORM_DOMAIN = domain;
config.PLATFORM_SUBDOMAIN_SUFFIX = await ask(rl, 'App subdomain suffix', existing.PLATFORM_SUBDOMAIN_SUFFIX || `.apps.${domain}`);

// ── Section 3: MongoDB ────────────────────────────────────────────────────────
console.log(`\n${BOLD}3. MongoDB${RESET}`);
if (!skip('MONGODB_URI')) {
  config.MONGODB_URI = await ask(rl, 'MongoDB URI', existing.MONGODB_URI || 'mongodb://127.0.0.1:27017/hellodeploy');
}

// ── Section 4: Redis ──────────────────────────────────────────────────────────
console.log(`\n${BOLD}4. Redis${RESET}`);
if (!skip('REDIS_HOST')) config.REDIS_HOST = await ask(rl, 'Redis host', existing.REDIS_HOST || '127.0.0.1');
if (!skip('REDIS_PORT')) config.REDIS_PORT = await ask(rl, 'Redis port', existing.REDIS_PORT || '6379');
...
if (!skip('RESEND_API_KEY')) {
  config.RESEND_API_KEY = await ask(rl, 'Resend API key (re_...)', existing.RESEND_API_KEY || '', true);
}
if (!skip('EMAIL_FROM')) {
...
if (!skip('GITHUB_APP_ID'))   config.GITHUB_APP_ID   = await ask(rl, 'App ID',   existing.GITHUB_APP_ID   || '');
if (!skip('GITHUB_APP_NAME')) config.GITHUB_APP_NAME = await ask(rl, 'App slug', existing.GITHUB_APP_NAME || '');
if (!skip('GITHUB_APP_PRIVATE_KEY_PATH')) {
  config.GITHUB_APP_PRIVATE_KEY_PATH = await ask(rl, 'Private key path', existing.GITHUB_APP_PRIVATE_KEY_PATH || '/etc/hellodeploy/github-app.pem');
}
if (!skip('GITHUB_WEBHOOK_SECRET')) {
  config.GITHUB_WEBHOOK_SECRET = await ask(rl, 'Webhook secret', existing.GITHUB_WEBHOOK_SECRET || '(will be generated)', true);
}
...
if (!skip('TURNSTILE_SITE_KEY'))   config.TURNSTILE_SITE_KEY   = await ask(rl, 'Site key',   existing.TURNSTILE_SITE_KEY   || '');
if (!skip('TURNSTILE_SECRET_KEY')) config.TURNSTILE_SECRET_KEY = await ask(rl, 'Secret key', existing.TURNSTILE_SECRET_KEY || '', true);

// ── Section 8: worker paths ───────────────────────────────────────────────────
console.log(`\n${BOLD}8. Worker directories${RESET}`);
if (!skip('BUILD_WORKSPACE_ROOT'))  config.BUILD_WORKSPACE_ROOT  = await ask(rl, 'Build workspace', existing.BUILD_WORKSPACE_ROOT  || '/var/lib/hellodeploy/builds');
if (!skip('RELEASE_METADATA_ROOT')) config.RELEASE_METADATA_ROOT = await ask(rl, 'Release metadata', existing.RELEASE_METADATA_ROOT || '/var/lib/hellodeploy/releases');
if (!skip('PROJECT_VOLUME_ROOT'))   config.PROJECT_VOLUME_ROOT   = await ask(rl, 'Project volumes',  existing.PROJECT_VOLUME_ROOT   || '/var/lib/hellodeploy/projects');
if (!skip('NGINX_HELLODEPLOY_CONFIG_DIR')) {
  config.NGINX_HELLODEPLOY_CONFIG_DIR = await ask(rl, 'Nginx config dir', existing.NGINX_HELLODEPLOY_CONFIG_DIR || '/etc/nginx/hellodeploy.d');
}
...
const genResult = spawnSync(process.execPath, [resolve(__dirname, 'generate-secrets.js'), '--write', '--output', ENV_PATH], {
  encoding: 'utf8',
  env: { ...process.env },
});
...
console.log(`     sudo mkdir -p ${config.NGINX_HELLODEPLOY_CONFIG_DIR || '/etc/nginx/hellodeploy.d'}`);
console.log(`     echo 'include ${config.NGINX_HELLODEPLOY_CONFIG_DIR || '/etc/nginx/hellodeploy.d'}/*.conf;' | sudo tee /etc/nginx/conf.d/hellodeploy.conf`);
